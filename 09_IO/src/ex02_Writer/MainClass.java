...
public class MainClass {
	...
	public static void ex01() {
		...
		if(dir.exists() == false) { // 만약 디렉터리가 없으면
			dir.mkdirs();			// 디렉터리를 만들어라
		}
		...
		try {
			...
			e.printStackTrace(); // 개발할 때 캐치코드는 이걸로 통일한다.
		}finally { // 예외가 발생 하든 안 하든 발생하는 코드 
			try {
				if(fw != null) {
	...
	public static void ex04() {
		...
		// try - catch - resources문을 이용해서  ex03을 바꿔서 실행 해보기 
		File dir = new File("C:" + File.separator + "storage");
		if(dir.exists()) {
			dir.mkdirs();
		}
		File file = new File(dir,"ex04.txt");
		// 메인 스트림하고 보조 스트림 함께 사용
