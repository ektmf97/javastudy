...
public class MainClass {
	/*
		 	...
		 	3. 주로 Key는 String을 사용한다. (키의 변수이름)
	...
	public static void ex01() {
		...
		// 삭제
		// remove(Key) // 키값을 전달하면 해당 엔트리에 키값을 삭제 시켜준다.
	...
	public static void ex02() {
		...
		Map<String, String> dictionaty = new HashMap<String, String>();
		dictionaty.put("spring","봄");
		dictionaty.put("summer","여름");
		dictionaty.put("autumn","가을");
		dictionaty.put("winter","겨울");
		
		// Value 가져오기
		// get(Key)
		String season = dictionaty.get("winter");
	...
	public static void ex03() {
		...
		// 1. 첫 번째 for문 돌리기
		// (키만 분리가능), set를 사용
		// Key만 모두 가져온 뒤, 해당 Key값을 가진 Value를 가져오기
		Set<String> keySet = dictionaty.keySet();
		for(String key : keySet) {
			System.out.println(key + ":" + dictionaty.get(key) );
		}
		
		// 2. 두 번째 for문 돌리기
		// Entry를 모두 가져온 뒤, Key와 Value로 분리하기 
		for(Entry<String, String> entry : dictionaty.entrySet()) {
	...
	public static void ex04() {
		...
		for(int i = 0 , length = books.size(); i < length; i++) {
			...
			Map<String, Object> book = books.get(i); // books[i]
