...
public class Library {
	...
	private void deleteBook() { // RuntimeException은 UnCheckde - Exception이므로 throw을 생략할 수 있다.
		System.out.println("=== 책 삭제하기 ===");
			if(books.isEmpty()) {
				throw new RuntimeException("등록된 책이 없습니다."); // 예외처리 
			}
			System.out.println("삭제할 isbn >>>");
			String isbn = sc.next();
			if(isbn.isBlank() == false) { // 비어 있지 않을때 
				...
				/* 2. 인덱스 기반 삭제
				for(int i = 0; i < books.size(); i++) { // 향상 for문은 인덱스가 없다.
					Book book = books.get(i);
					if(isbn.equals(book.getIsbn())) {
						Book deletedBook = books.remove(i); // 이렇게도 사용가능 
						System.out.println(deletedBook + "책이 삭제되었습니다.");
						return;
					//	books.remove(i); // remove(i)는 삭제된 요소(Book)를 반환한다.
